...
export function printBackCrewSection() {
  ...
  CREW_ADD_BTN.addEventListener("click", (e) => {
    ...
    if (BACK_CREW_OBJ.findIndex((name) => name === CREW_NAME) !== -1) {
      window.alert("[ERROR] 중복된 이름이 있습니다.");
      return;
    }

    BACK_CREW_OBJ.push(CREW_NAME);
    window.localStorage.setItem("back_crew", JSON.stringify(BACK_CREW_OBJ));
    printAddCrew(BACK_CREW_OBJ.length, CREW_NAME);
  });
...
function listingSection() {
  ...
  BACK_CREW_OBJ.forEach((name, index) => {
    const INDEX = index + 1;
  ...
  for (let btn of Array.from(DELETE_BTNS)) {
    btn.addEventListener("click", (e) => {
      ...
      const index = BACK_CREW_OBJ.findIndex((value) => value === NAME);
      BACK_CREW_OBJ.splice(index, 1);
